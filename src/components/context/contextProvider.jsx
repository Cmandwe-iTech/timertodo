...
const ContextProvider = (props) => {
  ...
  console.log(history);
    const signinUser = async () => {
        await axios.post("http://localhost:3005/login", input).then((res) => {
    ...
    const FetchData = () => {
        ...
        axios.get("http://localhost:3005/todopost", config).then((res) => {
    ...
    const CreateToDo = async () => {
        ...
        await axios.post("http://localhost:3005/todopost", activities, config).then((res) => {
            if (res.status === 200) {
                FetchData()
            }
        }).catch(e => {
    ...
    const update_Time = async (id1, st) => {
      ...
      await axios.put(`http://localhost:3005/todopost/${id1}`, {time_taken:`${hr} : ${min}: ${sec}`, Status:st},config).then((res)=>{
        if(res.status === 200){
          FetchData()      
        }
      }).catch((e)=>{
    ...
    const SignUpSubmit = async () => {
        if (validateUser(signup)) {
            await axios.post("http://localhost:3005/register", signup).then((res) => {
                if (res.status === 200) {
